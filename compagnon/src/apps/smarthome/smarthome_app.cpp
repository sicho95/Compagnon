...
// ─── Contrat AppDescriptor ───────────────────────────────────────────────────────
bool smarthome_app_is_running() { return _app_active; }
...
void smarthome_set_tuya_id(const char *id) {
    strlcpy(_tuya_id, id, sizeof(_tuya_id));
    nvs_set_api_key(NVS_KEY_TUYA_ID, id);
}
void smarthome_set_tuya_secret(const char *sec) {
    strlcpy(_tuya_sec, sec, sizeof(_tuya_sec));
    nvs_set_api_key(NVS_KEY_TUYA_SEC, sec);
}
...
static void parse_status(int idx, const char *json) {
    ...
    for (JsonObject s : arr) {
        ...
        if      (strcmp(code, "switch_1")        == 0 ||
                 strcmp(code, "switch_led")       == 0 ||
                 strcmp(code, "switch")           == 0)
            _devices[idx].power = s["value"] | false;
        else if (strcmp(code, "bright_value")    == 0 ||
                 strcmp(code, "bright_value_v2") == 0)
            _devices[idx].brightness = s["value"] | 0;
        else if (strcmp(code, "va_temperature")  == 0 ||
                 strcmp(code, "temp_current")     == 0)
            _devices[idx].temp     = (float)(s["value"] | 0) / 10.0f;
        else if (strcmp(code, "va_humidity")     == 0 ||
                 strcmp(code, "humidity_value")   == 0)
...
static void on_fetch_done(void *) {
    ...
    // Construire les cartes au premier fetch
    if (_list_cont && lv_obj_get_child_count(_list_cont) == 0) {
        int card_w = (UI_W - 24) / 2;
        for (int i = 0; i < _dev_count && i < MAX_DEVICES; i++) {
            ...
            if (!_devices[i].is_sensor && _devices[i].online) {
                ...
                lv_obj_add_event_cb(card, [](lv_event_t *e) {
                    ...
                    const char *code = (strncmp(_devices[idx].category, "dj", 2) == 0)
                                       ? "switch_led" : "switch_1";
...
static void fetch_task(void *) {
    ...
    if (_dev_count == 0) {
        ...
        if (buf && tuya_api_get_devices(buf, 8192)) parse_devices(buf);
        free(buf);
    }
...
static void do_close() {
    ...
    for (int i = 0; i < MAX_DEVICES; i++) {
        _dev_cards[i]  = nullptr;
        _lbl_power[i]  = nullptr;
        _lbl_sensor[i] = nullptr;
    }
    ui_launcher_return();
