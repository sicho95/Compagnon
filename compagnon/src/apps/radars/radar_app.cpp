...
static lv_obj_t *_scr_radar = nullptr;

static void back_cb_radar(lv_event_t *) {
    orchestrator_set_app(APP_LAUNCHER);
    lv_obj_del(_scr_radar); _scr_radar = nullptr;
    ui_launcher_return();
}
...
void radar_app_start() {
    ...
    lv_obj_set_style_bg_color(_scr_radar, lv_color_hex(0x0A0A1A), 0);
    ...
    lv_obj_add_event_cb(btn, back_cb_radar, LV_EVENT_CLICKED, NULL);
    lv_obj_t *lb = lv_label_create(btn);
    lv_label_set_text(lb, LV_SYMBOL_LEFT);
    lv_obj_set_style_text_color(lb, lv_color_hex(0x7EB8F7), 0);
    lv_obj_center(lb);

    lv_obj_t *title = lv_label_create(_scr_radar);
    ...
    lv_obj_t *body = lv_label_create(_scr_radar);
    lv_label_set_text(body, "Scan RF / LoRa\n\n(en cours de developpement)");
    lv_obj_set_style_text_font(body, &lv_font_montserrat_16, 0);
    lv_obj_set_style_text_color(body, lv_color_hex(0x7EB8F7), 0);
    lv_obj_set_style_text_align(body, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_width(body, 400);
    lv_obj_align(body, LV_ALIGN_CENTER, 0, 0);

    lv_scr_load_anim(_scr_radar, LV_SCR_LOAD_ANIM_MOVE_LEFT, 300, 0, false);
    Serial.println("[APP/RADARS] Ouverte");
}

void radar_app_stop() {
    if (_scr_radar) { lv_obj_del(_scr_radar); _scr_radar = nullptr; }
    orchestrator_set_app(APP_LAUNCHER);
}
