...
static lv_obj_t *_scr_bourse = nullptr;

static void back_cb_bourse(lv_event_t *) {
    orchestrator_set_app(APP_LAUNCHER);
    lv_obj_del(_scr_bourse); _scr_bourse = nullptr;
    ui_launcher_return();
}
...
void bourse_app_start() {
    ...
    lv_obj_set_style_bg_color(_scr_bourse, lv_color_hex(0x061A06), 0);
    ...
    lv_obj_set_style_bg_color(btn, lv_color_hex(0x061A06), 0);
    lv_obj_set_style_radius(btn, 10, 0);
    lv_obj_add_event_cb(btn, back_cb_bourse, LV_EVENT_CLICKED, NULL);
    lv_obj_t *lb = lv_label_create(btn);
    lv_label_set_text(lb, LV_SYMBOL_LEFT);
    lv_obj_set_style_text_color(lb, lv_color_hex(0x66EE88), 0);
    lv_obj_center(lb);

    lv_obj_t *title = lv_label_create(_scr_bourse);
    lv_label_set_text(title, "Bourse");
    lv_obj_set_style_text_font(title, &lv_font_montserrat_24, 0);
    lv_obj_set_style_text_color(title, lv_color_hex(0x66EE88), 0);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 46);

    lv_obj_t *body = lv_label_create(_scr_bourse);
    lv_label_set_text(body, "Marches & Portefeuille\n\n(en cours de developpement)");
    lv_obj_set_style_text_font(body, &lv_font_montserrat_16, 0);
    lv_obj_set_style_text_color(body, lv_color_hex(0x66EE88), 0);
    lv_obj_set_style_text_align(body, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_width(body, 400);
    lv_obj_align(body, LV_ALIGN_CENTER, 0, 0);

    lv_scr_load_anim(_scr_bourse, LV_SCR_LOAD_ANIM_MOVE_LEFT, 300, 0, false);
    Serial.println("[APP/BOURSE] Ouverte");
}

void bourse_app_stop() {
    if (_scr_bourse) { lv_obj_del(_scr_bourse); _scr_bourse = nullptr; }
    orchestrator_set_app(APP_LAUNCHER);
}
